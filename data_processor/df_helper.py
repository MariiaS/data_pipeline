...
def add_address_data_to_df(dataframe) -> pd.DataFrame:
    ...
    for i, row in dataframe.iterrows():
        geo_json = geo_helper.get_address_from_ip(dataframe.ip_address[i])
...
def add_country_column_to_df(dataframe) -> pd.DataFrame:
    ...
    for i, row in dataframe.iterrows():
        ...
        country = geo_helper.get_country_from_city_state(location_row['city'], location_row['street'], location_row['postcode'])
